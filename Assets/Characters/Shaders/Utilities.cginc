...
// half3 RimLighting(float3 normal, float3 smoothNormal, float nBlend, float rimLength, float rimLightWidth, float curDepth,half3 color, float3 positionSS, float3 viewDir, float3 lightDir)
// {
...
//     float2 ssUV = positionSS + normalVS * lDotN * rimLightWidth * color.b * 40/*GetSSRimScale(curDepth)*/;
//     float depth = tex2D(_CameraDepthTexture,clamp(ssUV, 0, _ScreenParams.xy - 1));
//     float depthScene = LinearEyeDepth(depth);
//     float depthDiff = depthScene - curDepth;
//     float intensity = smoothstep(0.24 * _RimLightFeather *curDepth, 0.25 * curDepth, depthDiff);
